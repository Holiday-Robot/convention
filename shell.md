...
스크립트를 실행하는 방법에는 두 가지가 있으며, 동작 방식이 근본적으로 다릅니다.

| 구분          | 실행 (`bash script.sh`, `./script.sh`) | source (`. script.sh`, `source script.sh`) |
| ------------- | -------------------------------------- | ------------------------------------------ |
| 실행 환경     | 새로운 자식 프로세스(subshell)         | 현재 셸 프로세스                           |
| 환경 변수     | 변경이 부모 셸에 반영되지 않음         | 변경이 현재 셸에 직접 반영됨               |
| 작업 디렉터리 | 변경이 부모 셸에 반영되지 않음         | 변경이 현재 셸에 직접 반영됨               |
| shebang       | 인터프리터를 결정하는 데 사용됨        | 무시됨                                     |
| 실행 권한     | 필요함                                 | 불필요함                                   |

스크립트를 실행하면 자식 프로세스에서 동작하므로, 스크립트 내에서 변경한 환경 변수나 현재 디렉터리는 부모 셸에 영향을 주지 않습니다.
...
# 실행 후 부모 셸에서 확인
bash script.sh
echo "$FOO"  # 출력 없음 — 변경이 반영되지 않음
pwd          # /tmp가 아님
```

반면 `source`는 현재 셸에서 직접 실행되므로, 환경 변수 설정이나 함수 정의처럼 현재 셸 상태를 변경할 목적으로 사용합니다.
...
`source`를 염두에 두고 작성된 파일은 호출 시 부작용이 없어야 합니다.
실행 가능한 스크립트에 `main` 함수 패턴을 적용하는 이유이기도 합니다.
`source`로 불러오면 함수만 정의되고 실행은 일어나지 않지만, 직접 실행하면 `main "$@"`가 호출됩니다.
